        ...
        >
          {data.map((item) => (
            <SwiperSlide key={item.id} className="!w-[82%] relative md:mx-[25px] mx-[10px] rounded-[10px] overflow-hidden h-auto">
              {item.comming &&
                <div className="absolute inset-0 bg-gray-400/65 flex items-center justify-center text-[40px] md:text-[100px] z-50 text-white italic">
                  Coming soon!
                </div>
              }
                ...
                <div className="slide-content px-5 md:px-10 xl:px-[64px] md:py-8 py-6 xl:py-[54px] flex relative gap-5 xl:gap-[70px] items-center max-md:flex-col" style={{minHeight: slideHeight > 0 ? `${slideHeight}px` : 'auto'}}>
                    <div className="max-md:w-full max-xl:w-1/2 [box-shadow:0px_4px_100px_0px_rgba(97,_169,_197,_0.25)]">
                    {item.image ?
                    ...
                    <div className="text-white flex-1">
                        <p className="md:text-[24px] font-medium text-[18px] leading-[1.2]">{item.name && item.name}</p>
