...
const MediaItem: React.FC<MediaItemProps> = ({ item, index }) => {
  ...
  useEffect(() => {
    ...
    const speedFactor = 0.03 + (Math.random() * 0.04);
    const depthFactor = 5 + (Math.random() * 15);
...
const Fv: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const createMarqueeAnimation = (selector: string, xPercentValue: number): void => {
  ...
  return (
    <main ref={containerRef} className="min-h-screen p-4  overflow-hidden bg-black">
      <div className="flex flex-col md:justify-between justify-center bg-[url(/assets/images/svg-deco.svg)] bg-no-repeat inset-0 h-screen bg-center py-[10vw] max-md:gap-[30vw] md:py-[130px]">
      <Marquee 
        items={MEDIA_ITEMS} 
        className="marquise-right-1 mr-[-45%]" 
      />
      ...
      <div className="w-full absolute inset-0 ">
        ...
        >
          ...
          >
            Unleashing Global <br />Entertainment Value <br />Across Borders 
          </h4>
        </div>
      </div>
    </main>
