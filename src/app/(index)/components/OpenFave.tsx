...
const data = [
    {
        ...
        title: (<>投稿そのものが資産</>),
        content: (<>投稿したデータがNFT化されることで、「所有・投資・収集」できる新しい楽しみ方へと進化します。<br />あなた自身がビジュアルアートの持ち主となり、その作品の物語に深く関わることができるのです。</>)
    },
    {
        image: '/assets/images/open-fave-img-02.png',
        title: (<>画像だけじゃない、様々なデータを簡単にNFT化</>),
        content: (<>
        写真やイラストはもちろん、音声、動画、PDFなど、あらゆるデジタルデータを簡単にNFT化できます。新たな発信・共有・応援のスタイルが実現します。
        </>)
    },
    {
        ...
        content: (<>
        OPEN FAVEのNFTには、誰もが閲覧できる「A面」と、所有者だけが見ることのできる「B面」が存在します。<br />
一般公開されるA面で作品の魅力を伝えつつ、B面はコレクター専用の特別な要素やコンテンツを提供。<br />
これによって、NFTの所有者は他者には見えない価値や物語を独占的に楽しめ、作品への愛着や満足度をさらに高めることができます。
        </>)
    },
    {
        image: '/assets/images/open-fave-img-04.png',
        title: (<>今までのSNSにはなかった閲覧者としての価値も</>),
        content: (<>多彩なデジタルコンテンツをNFTとして所有することで、今までのSNSには存在しなかった「閲覧者としての価値」を手に入れられます。
            "見る・聞く"から"持つ"へ。
