...
export const Revenue = ({ isActive }: { isActive: boolean }) => {
  ...
  return (
    <section className="w-full h-full overflow-hidden">
      <div
        ref={ref}
        className="w-full h-full flex items-center justify-center md:pl-[160px] xl:px-[160px] pl-5 pr-5"
      >
        <div className="w-full max-w-[1050px]">
          ...
          <div className="flex justify-center pt-10 md:pt-[min(5.5vmin,44px)] pb-10 md:pb-[min(3.7vmin,30px)] gap-3 md:gap-[min(3.7vmin,30px)] max-md:flex-col max-md:w-[320px] max-md:mx-auto">
            {data.map((item, index) => (
              <div
                key={index}
                className="fade-item w-[320px] md:h-[min(40vmin,320px)] p-[min(1vmin,15px)] flex items-center justify-center flex-col border [&:nth-of-type(1)]:border-[#2EDCFF] [&:nth-of-type(1)]:bg-[#2EDCFF]/[0.1] [&:nth-of-type(2)]:border-[#DB00B2] [&:nth-of-type(2)]:bg-[#DB00B2]/[0.1] [&:nth-of-type(3)]:border-[#6F00FF] [&:nth-of-type(3)]:bg-[#6F00FF]/[0.1] rounded-[20px] max-md:p-5"
              >
                <p className="font-bold text-center leading-loose text-[16px] xl:text-[min(3vmin,24px)] md:text-[min(2.5vmin,18px)] mb-3 md:mb-[min(3vmin,24px)]">
                  {item.title}
                </p>
                <ul className="text-[14px] xl:text-[min(2.5vmin,20px)] md:text-[min(2vmin,14px)] md:space-y-[min(2vmin,15px)] space-y-1 font-medium max-md:max-w-[240px] max-md:mx-auto max-md:w-full">
                  {item.list.map((listItem, listIndex) => (
                    <li key={listIndex} className="flex items-center gap-2">
                      <span>
                        <img className="max-md:w-4 max-lg:w-4" src="/assets/images/check-circle.svg" alt="" />
                      </span>
          ...
          <div className="fade-item w-full max-w-[544px] mx-auto max-md:pb-10">
