    ...
    <section className="w-full h-full overflow-hidden">
      ...
      >
        <div className="w-full max-w-[1200px]">
            ...
            <div className="flex justify-center pt-[min(5.5vmin,44px)] pb-[min(3.7vmin,30px)] max-md:flex-col max-md:items-center space-y-2 md:space-x-5 lg:space-x-14">
                <div className="fade-item flex items-center">
                    <p className="font-bold md:text-[min(2.5vmin,20px)] text-[min(4vmin,18px)] max-md:text-center whitespace-nowrap">
                        トークン概要
                    </p>
                </div>
                    ...
                    <ul className="md:text-[min(2vmin,16px)] text-[min(2.5vmin,14px)] leading-loose">
                        {
                            [
                                ...
                                <li key={index} className="flex md:gap-12 border-b border-[#5F5F5F] gap-12 mb-[min(2.5vmin,12px)]">
                                    <span className="lg:text-[min(2.5vmin,20px)] text-[min(3vmin,16px)] font-bold w-[130px] lg:w-[174px]">{item.title}</span>
                                    <span className="lg:text-[min(3vmin,24px)] text-[min(2vmin,20px)]">{item.value}</span>
                                </li>
                            ))
                        }
                    </ul>
