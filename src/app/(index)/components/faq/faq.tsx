...
const faqData: FaqItemPropsType[] = [
  ...
  },
  {
    ...
    answer: (
      <>
        プランによって料金は異なります。詳細は料金ページをご確認ください。なお、メタバース部分の価格設定には、最新の技術開発、カスタマイズされたペットアバター制作、AI対話機能実装など、高度な技術と専門知識を要する多様な要因が含まれています。
        <br />
        決して安価とは言えませんが、現実の供養の場合、一度〜複数回の供養をするのみで同程度の費用がかかります。それでいて、段々と“おもいで”は風化していきます。この辛さと向き合い、乗り越えることは容易ではありません。メタバースなら、好きなタイミングでのペットとの再会と定期的な供養体験により、喜びと弔いの気持ちを同時に叶えられます。
        <br />
  ...
  },
  {
    ...
    answer: (
      <>
        はい、犬や猫以外の特殊なペットについても、ご相談を承っております。ただし、現在はメモリアルモニタープランでのお申し込みをいただいた方限定の特典となっております。メモリアルモニタープランをご検討いただければ、どんなペットでも受付可能です。詳細については、お気軽にお問い合わせください。
        <br />
        ※対象のペットちゃんのご参考例
        <br />
