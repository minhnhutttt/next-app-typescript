...
const faqData: FaqItemPropsType[] = [
  ...
  },
  {
    ...
    answer: (
      <>
        現在はベータ版をセール価格で販売中です。詳細はページトップをご参照ください。なお、メタバース部分の価格設定には、最新の技術開発、カスタマイズされたペットアバター制作、AI対話機能実装など、高度な技術と専門知識を要する多様な要因が含まれています。決して安価とは言えませんが、現実の供養の場合、一度〜複数回の供養をするのみでそれ以上の費用がかかります。それでいて、段々と“おもいで”は風化していきます。この辛さと向き合い、乗り越えることは容易ではありません。メタバースなら、好きなタイミングでのペットとの再会と定期的な供養体験により、喜びと弔いの気持ちを同時に叶えられます。セール価格は予告なく変更または停止される場合がございます。ぜひ、今の機会にお申し込みをいただけますと幸いです。
      </>
    ),
  },
  {
