...
export default function FaqItem({ id, question, answer }: FaqItemPropsType) {
  ...
  const handleExpanderClick = () => {
    ...
    if (isClosedRef.current) {
      ...
      gsap.from($content, { duration: 0.2, height: 0, onComplete: ScrollTrigger.refresh });
      setIsClose(false);
    } else {
      gsap.to($content, { duration: 0.2, height: 0, onComplete: ScrollTrigger.refresh });
      setIsClose(true);
    }
    ...
    <div className="fade-up px-3 md:px-[25px] py-9 md:py-[50px]">
      ...
      >
        ...
        <div className="flex justify-between">
          <p className="text-left text-[20px] font-bold md:text-[32px]">{question}</p>
          <div className="relative ml-2 flex size-8 items-center justify-center duration-150 md:flex-[0_0_51px] flex-[0_0_32px] md:size-[51px]">
      ...
      <div ref={contentRef} className="h-0 overflow-hidden">
        <p className="md:text-[28px] text-[20px] leading-[1.4] md:pt-9 pt-5">{answer}</p>
      </div>
    </div>
  );
}
