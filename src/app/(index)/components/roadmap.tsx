...
const Phase = ({phase, items, current = false}: {phase: string, items: string[], current?: boolean}) => (
    <div className="flex items-center md:py-[50px] py-8 max-md:gap-5 border-b border-white">
            <div className="xl:w-[480px] flex justify-center items-center flex-col leading-[1.3]">
                <span className="md:text-[48px] text-[4vw] font-bold text-center whitespace-nowrap">Phase {phase}</span>
...
useEffect(() => {
  ...
  return (
    ...
    <section ref={containerRef} id="brain" className="road-prerspective max-md:mb-[0] max-xl:mb-[400px] relative pt-20 md:pt-[50px] md:pb-10 pb-[60px] px-8">
      <div className="road-move relative w-full max-w-[1310px] mx-auto">
        ...
        <div className="md:mt-[100px] mt-[70px]">
            {PhaseData.map((phase, index) => (
                <Phase phase={phase.number} items={phase.items} current={phase.current} key={index} />
            ))}
        </div>
      </div>
