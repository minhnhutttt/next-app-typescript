...
const InfiniteImageGrid: React.FC<InfiniteImageGridProps> = ({
  ...
  const loadedMediaCountRef = useRef(0)
  const totalMediaCount = new Set(mediaItems.map((item) => item.src)).size
  ...
  const handleMediaLoaded = () => {
    loadedMediaCountRef.current += 1

    if (
      loadedMediaCountRef.current >= totalMediaCount ||
      loadedMediaCountRef.current >= mediaItems.length * rowNum
    ) {
      onLoadComplete?.()
    }
  }
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (loadedMediaCountRef.current < totalMediaCount) {
        onLoadComplete?.()
      }
    }, 3000)
  ...
  }, [onLoadComplete, totalMediaCount])
    ...
    if (rowIndex < rowMidIndex) {
      for (let i = rowIndex; i < rowMidIndex; i++) {
        ...
        if (rowArray.length % 2 === 1) {
          ...
          const lastMatrix = new DOMMatrix(
            lastRowTransform === 'none'
              ? 'matrix(1, 0, 0, 1, 0, 0)'
              : lastRowTransform
          )
      ...
      for (let i = rowMidIndex; i < rowIndex; i++) {
        ...
        if (rowArray.length % 2 === 1) {
          const firstRowTransform = window.getComputedStyle(firstRow).transform
          const firstMatrix = new DOMMatrix(
            firstRowTransform === 'none'
              ? 'matrix(1, 0, 0, 1, 0, 0)'
              : firstRowTransform
          )
  ...
  const updateCenterElem = () => {
    ...
    for (const elem of elems) {
      if (
        elem.classList.contains('sliderImage') &&
        elem !== lastCenteredElemRef.current
      ) {
  ...
  const handleDragMove = (e: MouseEvent) => {
    ...
    if (elapsed > 0) {
      ...
      velocityRef.current = {
        x:
          velocityRef.current.x * (1 - velocityBlendFactor) +
          rawVelocityX * velocityBlendFactor,
        y:
          velocityRef.current.y * (1 - velocityBlendFactor) +
          rawVelocityY * velocityBlendFactor,
      }
    }
  ...
  const resize = () => {
    ...
    const horizOffset =
      -(imgMidIndex * horizSpacing + boxWidth / 2) + windowWidth / 2
    const vertOffset =
      -(rowMidIndex * vertSpacing + boxHeight / 2) + windowHeight / 2
    ...
    rowsRef.current.forEach((row, i) => {
      ...
      const images = Array.from(
        row.querySelectorAll('.sliderImage')
      ) as HTMLDivElement[]
    ...
    if (
      rowsRef.current.length > rowMidIndex - 1 &&
      rowsRef.current[rowMidIndex - 1]?.querySelectorAll('.sliderImage')
        .length > imgMidIndex
    ) {
  ...
  useEffect(() => {
    imgRepRef.current = Array(rowNum)
      .fill(0)
      .map(() => [])
  ...
  useEffect(() => {
    ...
    const touchMoveHandler = (e: Event) => {
      ...
      if (initialTouchRef.current) {
        ...
        if (
          touchDirectionDeterminedRef.current &&
          !shouldHandleTouchRef.current
        ) {
          return
        }
    ...
    if (overlayElement) {
      overlayElement.addEventListener('touchstart', touchStartHandler, {
        passive: true,
      })
      overlayElement.addEventListener('touchmove', touchMoveHandler, {
        passive: false,
      })
      overlayElement.addEventListener('touchend', touchEndHandler, {
        passive: true,
      })
      overlayElement.addEventListener('touchcancel', touchEndHandler, {
        passive: true,
      })
    }
  ...
  return (
    <>
      <div
        ref={wrapperRef}
        className="relative h-screen w-full cursor-grab overflow-hidden"
        style={{ touchAction: 'pan-y' }}
      >
        <div
          className="touch-overlay absolute inset-0 z-10"
        ...
        <div className="h-screen w-full overflow-hidden">
          <div
            ref={containerRef}
            className="inset-0 ease-linear will-change-transform"
          >
                  ...
                  {Array.from({ length: imgNum }).map((_, imgIndex) => {
                    ...
                    return (
                      <div
                        ...
                        className="sliderImage absolute left-0 top-0 overflow-hidden rounded-[20px]"
                      >
