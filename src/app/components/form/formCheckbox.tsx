...
export default function FormCheckbox({ label, name }: CheckboxProps) {
  ...
  return (
    <div ref={animateRefs} className="opacity-0 flex items-center gap-2">
      <label className="relative flex items-center px-1 py-3 md:p-3 rounded-full cursor-pointer" htmlFor={name}>
        <input
          type="checkbox"
      ...
      <label className="mt-px text-[13px] md:text-[18px] font-bold cursor-pointer select-none tracking-widest" htmlFor={name}>
        {label}
      </label>
    </div>
  );
