  ...
  function IntroductionItem({ number, image,title, children }: IntroductionItemPropsType) {
    return (
      <div className="relative bg-[url('/images/introduction-bg.png')] bg-[length:100%_100%] bg-center max-w-[400px] px-10 md:px-[50px] pb-[40px] pt-[30px]">
        ...
        <p className="md:text-[18px] text-[15px] font-bold mt-6 leading-relaxed">{title}</p>
        <p className="md:text-[16px] text-[13px] font-medium mt-5">{children}</p>
        <span className="absolute bottom-0 right-0 leading-[0.8] md:text-[160px] text-[80px] text-[#FF9B9B]/[0.3] font-bold">{number}</span>
      </div>
    );
  }
          ...
          <div className="w-full max-w-[1320px] mx-auto pt-16 md:pt-[90px] md:pb-[128px] pb-20">
            ...
            <div className="mt-10 md:mt-[74px]">
                <div className="flex flex-wrap justify-center gap-8 md:gap-[60px] mb-8 md:mb-16">
                    <IntroductionItem number="01" image="/images/introduction-01.png" title="NFTがもたらす認証機関不要の即時証明力！">
                    認証機関を介さずに済むので、証明がすぐに完了。手間なく信頼を築けます。
                    </IntroductionItem>
                    <IntroductionItem number="02" image="/images/introduction-02.png" title="紙の証明書運用で発生している金銭的・時間的コストをまるごと削減！">
                    制作や郵送、保管のコストが不要に。経済的かつ迅速に業務を進められます。
                    </IntroductionItem>
                </div>
                <div className="flex flex-wrap justify-center gap-8 md:gap-[60px]">
                    <IntroductionItem number="03" image="/images/introduction-03.png" title="ニセモノを一発検出！なりすましや書き換えのリスクがゼロ！">
                    偽造や改ざんの心配なし。安全性が大幅に向上します。
                    </IntroductionItem>
                    <IntroductionItem number="04" image="/images/introduction-04.png" title="モバイル中心の運用でいつでもどこでもすぐさま確認&証明！">
                    スマホ一つで全てが完結。場所を選ばずに必要な情報を確認できます。
                    </IntroductionItem>
                    <IntroductionItem number="05" image="/images/introduction-05.png" title="紙の消費量減少に貢献！SDGsな組織運営で未来をアシスト！">
