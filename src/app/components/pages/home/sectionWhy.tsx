...
function ReasonCard({ number, title, active, content, explanation }: ReasonCardType) {
  return (
    <div className={`bg-white scale-[0.6] w-[300px] h-[600px] max-md:aspect-[380/545] md:h-[700px] md:w-full md:max-w-[400px] rounded-[40px] px-5 md:px-4 pt-5 md:pt-10 flex flex-col items-center justify-start border-8 border-[#3F3F3F] duration-500 ${!active && 'blur-sm opacity-40 pointer-events-none'}`}>
...
export default function SectionWhy() {
  ...
  const rotate = (direction: string) => {
    ...
    const newActiveSlide = direction === "n" ? (activeSlide < 8 ? activeSlide + 1 : 1) : (activeSlide === 1 ? 8 : activeSlide - 1);
              ...
              >
                <div className="absolute w-full [transform:rotateY(0deg)_translateZ(240px)] md:[transform:rotateY(0deg)_translateZ(400px)]">
                  <ReasonCard
                    ...
                    content={<>名刺管理アプリを使っているんですけど、紙の名刺を取り込もうとした時に撮影で完結するのはいいとして<span className="text-[#F3227A] font-bold">精度が粗い</span>んですよね。情報の誤登録が怖くていちいちチェックしたりするのが面倒でした。<span className="text-[#F3227A] font-bold">次世代名刺なら受け取った情報に誤りがない</span>ので、常に正しい情報にアクセスできます。安心感が違いますね。</>}
                ...
                <div className="absolute w-full [transform:rotateY(45deg)_translateZ(240px)] md:[transform:rotateY(45deg)_translateZ(400px)]">
                  <ReasonCard
                    ...
                    title="基本情報をワンタッチ起動"
                    content={<>名刺ってロゴや文字情報が基本じゃないですか。QRコードをプリントしたりすれば用途の幅は広がるけど、わざわざ読み込んだり、電話番号やメールアドレスを打ち込んだりする<span className="text-[#F3227A] font-bold">手間を相手に強いることになるのが嫌</span>で。次世代名刺なら電話やメール、サイトのリンクも<span className="text-[#F3227A] font-bold">ワンタッチでアクセスできて、相手の負担を物理的に減らせる</span>のがいいですね。住所のワンタッチでマップが起動する機能も便利でよく使ってます。</>}
                ...
                <div className="absolute w-full [transform:rotateY(90deg)_translateZ(240px)] md:[transform:rotateY(90deg)_translateZ(400px)]">
                  <ReasonCard
                    ...
                    title="着信時に名前が自動表示"
                    content={<>相手から次世代名刺を受け取ると、後日<span className="text-[#F3227A] font-bold">相手から着信があった際に次世代名刺アプリと連携して着信相手の名刺情報を表示してくれる</span>んですよ。電話帳の登録とかアプリの設定とか、何もしなくても自動で表示されたので最初はびっくりしましたが（笑）地味ですけど便利ですよね。</>}
                ...
                <div className="absolute w-full [transform:rotateY(135deg)_translateZ(240px)] md:[transform:rotateY(135deg)_translateZ(400px)]">
                  <ReasonCard
                    ...
                    title="充実した検索機能"
                    content={<>名刺ってどんどん貯まるものですよね。なので<span className="text-[#F3227A] font-bold">デジタル名刺の場合、検索機能が充実していることが重要</span>だと思っていて。次世代名刺なら、キーワード検索だけじゃなく、<span className="text-[#F3227A] font-bold">タグ付け機能</span>があるので、タグで名刺をグルーピングしたりして欲しい情報に楽にたどり着けるよう工夫してます。</>}
                ...
                <div className="absolute w-full [transform:rotateY(180deg)_translateZ(240px)] md:[transform:rotateY(180deg)_translateZ(400px)]">
                  <ReasonCard
                    ...
                    content={<><span className="text-[#F3227A] font-bold">自分の作品をどう売り込むか</span>って、自分みたいなアーティストにとっては死活問題だったりもするんですよね。でも、次世代名刺なら<span className="text-[#F3227A] font-bold">NFTに設定する画像を自分の作品サンプルのように使う</span>ことができると気付いたんです。それで気合い入れて作るようにして。実際に相手が興味を持ってくれた時は嬉しかったですよ。<span className="text-[#F3227A] font-bold">デザイン関係の方も同じように使える</span>と思います。</>}
                ...
                <div className="absolute w-full [transform:rotateY(225deg)_translateZ(240px)] md:[transform:rotateY(225deg)_translateZ(400px)]">
                  <ReasonCard
                    ...
                    content={<>普段<span className="text-[#F3227A] font-bold">経営者</span>の諸先輩方とお話しさせていただく機会が多いだけに、思いついた話のネタは常にストックしているのですが、次世代名刺の場合<span className="text-[#F3227A] font-bold">その真新しさやNFTならではの機能がそのまま話のネタになる</span>んですよね。相手が知らないと<span className="text-[#F3227A] font-bold">自動的に教える側に回れる</span>ので、コイツは役に立ちそうだと思ってもらえるのか可愛がってもらうことが増えました（笑）次世代名刺さまさまですよ。</>}
                ...
                <div className="absolute w-full [transform:rotateY(270deg)_translateZ(240px)] md:[transform:rotateY(270deg)_translateZ(400px)]">
                  <ReasonCard
                    ...
                    content={<>次世代名刺で発行されるNFT名刺は<span className="text-[#F3227A] font-bold">送信できる回数が1回だけで、受け取った相手が誰かに転送できない</span>仕様なんです。紙の名刺のように自分の名刺が知らない人に渡ってしまうことがなく、<span className="text-[#F3227A] font-bold">渡した相手から動くことがないので安心感</span>があります！なんていう機能だったか忘れちゃったけど、この機能は重宝しますね（笑）</>}
                ...
                <div className="absolute w-full [transform:rotateY(315deg)_translateZ(240px)] md:[transform:rotateY(315deg)_translateZ(400px)]">
                  <ReasonCard
                    ...
                    content={<><span className="text-[#F3227A] font-bold">紙名刺って実は年間で何百万本もの木が消費される</span>らしいんですよね。それを知ってから、<span className="text-[#F3227A] font-bold">SDGsを標榜しているのに木の消費から目を背けている矛盾</span>に気持ちが悪くなって（笑）思い切って次世代名刺の利用を始めたのですが、全面的に乗り換えるには時間も必要で。結局まだ紙名刺も併用しています。でも、少しでも<span className="text-[#F3227A] font-bold">持続可能な世界に貢献できることで矛盾が解消</span>されて気持ち的にはスッキリできましたよ。</>}
