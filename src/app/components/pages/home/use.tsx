...
function UseItem({ image, title, children, color }: UseItemPropsType) {
  ...
  return (
    <div
      ref={animateUpRefs}
      className="opacity-0 w-full md:w-[45%] max-w-[380px] md:max-w-[537px] aspect-square rounded-full"
    ...
    >
      <div className="xl:max-w-[350px] max-w-[75%] md:max-w-[28vw] w-full h-full mx-auto flex items-center justify-center flex-col">
        ...
        <h5 className="text-center md:text-[20px] lg:text-[24px] text-[18px] font-bold mt-5 lg:mt-8">
          {title}
        </h5>
        <p className="lg:text-[18px] text-[13px] font-medium mt-2 lg:mt-4 leading-relaxed">
          {children}
        </p>
      </div>
    </div>
  ...
  return (
    <section className="px-5 pt-10 md:pt-16 pb-24 md:pb-[155px]">
      <div className="w-full max-w-[380px] md:max-w-[1195px] mx-auto">
        <SectionTitle subTitle="現物資産×NFT">RWAタグでできること</SectionTitle>
        <div className="flex justify-center mt-7 md:mt-10">
          <UseItem
            image="/images/use-01.png"
            title="権利を譲渡する"
            color="#B7E7FF"
          >
