...
const data = [
    {
        image: '/images/img-sec-03-01.png',
        title: <>天然素材で安心！</>,
        text: <>手間と時間をかけた有機薬栽培で製造・商品化まで一貫して手掛けているため、<span className="font-black">安心安全</span>にお召し上がりいただけます。</>
    },
    {
        image: '/images/img-sec-03-02.png',
        title: <>ダイエットに最適！</>,
        text: <><span className="font-black">豊富な食物繊維</span>で満腹感を高め、<span className="font-black">ローカロリー！</span>もっちりと噛み応えもあるので<span className="font-black">ダイエット中の方にも自信をもって間食をオススメします。</span></>
    },
    {
        image: '/images/img-sec-03-03.png',
        title: <>食物繊維で健康！腸活！</>,
        text: <><span className="font-black">①腸内環境の改善 <br />
        ②便通の促進<br />
        ③血糖値の安定<br />
        ④コレステロールの低減</span></>
    },
...
const SectionHealth = () => {
  ...
  return (
    <section ref={ref} className="bg-[url('/images/bg-03.png')] bg-cover max-md:bg-bottom pt-8 md:pt-14 pb-40 md:pb-[530px]">
        <div className="fade-up w-full md:max-w-[1240px] max-w-[440px] mx-auto flex max-md:justify-center items-center md:gap-[60px] gap-5 px-5 pb-5 md:pb-7">
            <p className="">
                <svg className="md:h-[118px] h-[60px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 157 118" fill="none">
        ...
        <div className="md:pr-[15%] max-md:px-5">
            <div className="fade-up bg-[#E3BF3C]/[0.98] bg-[url('/images/bg-sec-03.png')] bg-cover px-5 md:px-[2vw] border-white border-2 md:border-l-0 pt-8 md:pb-[70px] md:pt-12 relative max-md:max-w-[440px] max-md:mx-auto">
                <div className="flex max-md:flex-col justify-between w-full max-w-[1150px] md:ml-auto gap-10 max-md:pb-10">
                    {data.map((item, index) => {
                        return (
                            <div className="fade-up flex-1 flex flex-col items-center" key={index}>
                                ...
                                <div className="mt-4 md:mt-6 text-white">
                                    <h5 className="text-center font-hiragino md:text-[26px] text-[16px] font-semibold leading-snug">
                                        {item.title}
                                    </h5>
                                    <p className="md:text-[20px] text-[13px] font-light md:mt-14 mt-7">
                                        {item.text}
                                    </p>
                                </div>
                            </div>
