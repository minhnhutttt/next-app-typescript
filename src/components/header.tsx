...
const Header = () => {
  ...
  return (
    <header className="flex items-center px-2 md:px-5 lg:px-[100px] h-[72px] z-40 bg-white relative">
          ...
          <ul className="flex lg:items-center max-lg:flex-col gap-6">
            {links.map(({ link, text, target }, index) => (
              <li key={index} className="flex items-center">
                <a
                  href={link}
                  className="text-[#0050A7] text-xl md:text-sm font-medium leading-tight"
                  target={target}
                  onClick={close}
                >
                  {text}
      ...
      <div className="absolute bottom-0 inset-x-0 h-px bg-[linear-gradient(90deg,_#D9748F_0%,_#0059BA_100%)]"></div>
    </header>
  );
};
