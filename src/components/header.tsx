...
const Header = () => {
  ...
  return (
    <header className="absolute w-full flex items-center px-5 py-7 z-40">
      <div className="w-full flex items-center justify-between h-20 md:h-[83px] bg-white/20 rounded-[10px]">
        ...
        <div
          className={`max-md:fixed max-md:left-0 max-md:top-0 max-md:h-screen max-md:w-full max-md:bg-white/90 z-[99] max-md:overflow-y-scroll flex items-center justify-center max-md:flex-col ${
          ...
          <ul className="flex md:items-center max-md:flex-col gap-6 px-4">
            {links.map(({ link, text, target }, index) => (
              <li key={index} className="flex items-center">
                <a
                  href={link}
                  className="text-[20px] md:text-[24px] md:text-[18px] font-black text-white duration-150 hover:opacity-75 tracking-widest h-[60px] flex items-center justify-center "
                  target={target}
                  onClick={close}
                >
                  {text}
