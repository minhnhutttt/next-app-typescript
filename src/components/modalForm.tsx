...
const ModalForm = () => {
  ...
  const extractLastFourDigits = (tel: string): string | null => {
    const match = tel.match(/(\d{4})$/)
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status == 200) {
        router.push(`/thx?id=${extractLastFourDigits(formData.tel)}`)
        closeModal()
      } else {
        console.error(res)
      ...
      <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
        <DialogPanel className="relative h-[90%] w-full max-w-[1040px] overflow-auto rounded-3xl border bg-white px-5 py-10 [box-shadow:0px_4px_60px_0px_rgba(0,_0,_0,_0.10)]">
          ...
          <form id="myForm" method="post">
            ...
            <div className="mt-5 flex justify-center max-md:text-[14px] md:mt-10">
              <div>
                <p>
                  <a href="/terms-of-service" className="underline">
                    利用規約
                ...
                <p className="mt-5 text-[12px] md:text-[13px]">
                  ※お申し込み前に内容をご確認ください。
                  <br />
                  お申し込みはあなた様の意思で行われたものとみなされます。
                </p>
              </div>
            </div>
          </form>
