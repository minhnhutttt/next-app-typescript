    ...
    <header className="absolute inset-x-0 top-0 w-full bg-white/5 p-4">
      <div className="mx-auto flex w-full max-w-[1320px] items-center justify-between">
        ...
        <nav
          id={navId}
          aria-label="Main"
          className={`text-white lg:static lg:block lg:flex-1 ${open ? 'max-lg:pointer-events-auto max-lg:opacity-100' : 'max-lg:pointer-events-none max-lg:opacity-0'} z-[99] transition-opacity duration-300 max-lg:fixed max-lg:inset-0 max-lg:bg-black/90`}
          aria-hidden={!open}
        >
        ...
        <button
          ...
          onClick={toggle}
          data-open={open}
          className={`group z-[100] h-5 w-10 lg:hidden ${open ? 'fixed top-5 right-5' : 'relative'} `}
        >
          <span
